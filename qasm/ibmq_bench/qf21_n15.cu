#include <stdio.h>
#include <mpi.h>
#include "../../src/util.h"
#include "../../src/dmsim_nvgpu_mpi.cuh"
//Use the NWQSim namespace to enable C++/CUDA APIs
using namespace NWQSim;

void prepare_circuit(Simulation &sim)
{
	sim.RZ(1.57079632679, 0);
	sim.SX(0);
	sim.RZ(1.57079632679, 0);
	sim.RZ(1.57079632679, 1);
	sim.SX(1);
	sim.RZ(1.57079632679, 1);
	sim.RZ(1.57079632679, 2);
	sim.SX(2);
	sim.RZ(1.57079632679, 2);
	sim.RZ(1.57079632679, 3);
	sim.SX(3);
	sim.RZ(1.57079632679, 3);
	sim.RZ(1.57079632679, 4);
	sim.SX(4);
	sim.RZ(1.57079632679, 4);
	sim.RZ(1.57079632679, 5);
	sim.SX(5);
	sim.RZ(1.57079632679, 5);
	sim.RZ(1.57079632679, 6);
	sim.SX(6);
	sim.RZ(1.57079632679, 6);
	sim.RZ(1.57079632679, 7);
	sim.SX(7);
	sim.RZ(1.57079632679, 7);
	sim.RZ(1.57079632679, 8);
	sim.SX(8);
	sim.RZ(1.57079632679, 8);
	sim.RZ(1.57079632679, 9);
	sim.SX(9);
	sim.RZ(1.57079632679, 9);
	sim.X(10);
	sim.X(12);
	sim.X(14);
	sim.RZ(1.57079632679, 12);
	sim.SX(12);
	sim.RZ(1.57079632679, 12);
	sim.CX(10, 12);
	sim.RZ(-0.785398163397, 12);
	sim.RZ(1.57079632679, 13);
	sim.SX(13);
	sim.RZ(1.57079632679, 13);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(8, 9);
	sim.CX(9, 8);
	sim.CX(8, 9);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(11, 14);
	sim.RZ(0.785398163397, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(12, 13);
	sim.RZ(0.785398163397, 12);
	sim.RZ(-0.785398163397, 13);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(11, 14);
	sim.CX(11, 14);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.RZ(1.57079632679, 10);
	sim.SX(10);
	sim.RZ(1.57079632679, 10);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.RZ(0.785398163397, 11);
	sim.RZ(1.57079632679, 11);
	sim.SX(11);
	sim.RZ(1.57079632679, 11);
	sim.CX(14, 13);
	sim.RZ(-0.785398163397, 13);
	sim.RZ(0.785398163397, 14);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(11, 14);
	sim.CX(13, 12);
	sim.RZ(-0.785398163397, 12);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(14, 13);
	sim.RZ(0.785398163397, 13);
	sim.CX(12, 13);
	sim.RZ(0.785398163397, 12);
	sim.RZ(-0.785398163397, 13);
	sim.CX(14, 13);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.RZ(0.785398163397, 12);
	sim.RZ(1.57079632679, 12);
	sim.SX(12);
	sim.RZ(1.57079632679, 12);
	sim.CX(12, 10);
	sim.RZ(1.57079632679, 10);
	sim.SX(10);
	sim.RZ(1.57079632679, 10);
	sim.RZ(1.57079632679, 12);
	sim.SX(12);
	sim.RZ(1.57079632679, 12);
	sim.CX(14, 13);
	sim.RZ(-0.785398163397, 13);
	sim.RZ(0.785398163397, 14);
	sim.CX(14, 13);
	sim.CX(13, 12);
	sim.RZ(-0.785398163397, 12);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(14, 13);
	sim.RZ(0.785398163397, 13);
	sim.CX(12, 13);
	sim.RZ(0.785398163397, 12);
	sim.RZ(-0.785398163397, 13);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(13, 12);
	sim.RZ(-0.785398163397, 12);
	sim.RZ(0.785398163397, 13);
	sim.CX(13, 12);
	sim.RZ(1.57079632679, 12);
	sim.SX(12);
	sim.RZ(1.57079632679, 12);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.RZ(0.785398163397, 14);
	sim.RZ(1.57079632679, 14);
	sim.SX(14);
	sim.RZ(1.57079632679, 14);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(14, 13);
	sim.RZ(-0.785398163397, 13);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(11, 14);
	sim.RZ(0.785398163397, 14);
	sim.CX(13, 14);
	sim.RZ(0.785398163397, 13);
	sim.RZ(-0.785398163397, 14);
	sim.CX(11, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(11, 14);
	sim.RZ(0.785398163397, 11);
	sim.RZ(0.785398163397, 13);
	sim.RZ(1.57079632679, 13);
	sim.SX(13);
	sim.RZ(1.57079632679, 13);
	sim.RZ(-0.785398163397, 14);
	sim.CX(11, 14);
	sim.CX(0, 1);
	sim.CX(1, 0);
	sim.CX(0, 1);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(0, 1);
	sim.CX(1, 0);
	sim.CX(0, 1);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(6, 7);
	sim.CX(7, 6);
	sim.CX(6, 7);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.RZ(-0.00306796157577, 5);
	sim.CX(5, 3);
	sim.RZ(0.00306796157577, 3);
	sim.CX(5, 3);
	sim.RZ(-0.00306796157577, 3);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.RZ(-0.00613592315154, 2);
	sim.CX(2, 1);
	sim.RZ(0.00613592315154, 1);
	sim.CX(2, 1);
	sim.RZ(-0.00613592315154, 1);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.0122718463031, 1);
	sim.CX(1, 0);
	sim.RZ(0.0122718463031, 0);
	sim.CX(1, 0);
	sim.RZ(-0.0122718463031, 0);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.0245436926062, 2);
	sim.CX(2, 3);
	sim.RZ(0.0245436926062, 3);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.0490873852123, 1);
	sim.CX(1, 4);
	sim.RZ(-0.0245436926062, 3);
	sim.RZ(0.0490873852123, 4);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.RZ(-0.0981747704247, 3);
	sim.RZ(-0.0490873852123, 4);
	sim.CX(4, 7);
	sim.CX(5, 8);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(6, 7);
	sim.CX(7, 6);
	sim.CX(6, 7);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.RZ(0.0981747704247, 5);
	sim.CX(3, 5);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.196349540849, 1);
	sim.CX(1, 4);
	sim.RZ(0.196349540849, 4);
	sim.CX(1, 4);
	sim.RZ(-0.196349540849, 4);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.RZ(-0.392699081699, 4);
	sim.CX(4, 7);
	sim.RZ(-0.0981747704247, 5);
	sim.RZ(0.392699081699, 7);
	sim.CX(4, 7);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(0, 1);
	sim.CX(1, 0);
	sim.CX(0, 1);
	sim.RZ(-0.785398163397, 3);
	sim.RZ(-0.392699081699, 7);
	sim.CX(6, 7);
	sim.CX(7, 6);
	sim.CX(6, 7);
	sim.CX(8, 9);
	sim.CX(9, 8);
	sim.CX(8, 9);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.RZ(0.785398163397, 5);
	sim.CX(3, 5);
	sim.RZ(-0.785398163397, 5);
	sim.RZ(-0.00613592315154, 5);
	sim.CX(8, 9);
	sim.CX(9, 8);
	sim.CX(8, 9);
	sim.CX(5, 8);
	sim.RZ(0.00613592315154, 8);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.RZ(-0.0122718463031, 3);
	sim.CX(3, 2);
	sim.RZ(0.0122718463031, 2);
	sim.CX(3, 2);
	sim.RZ(-0.0122718463031, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.RZ(-0.0245436926062, 2);
	sim.CX(2, 1);
	sim.RZ(0.0245436926062, 1);
	sim.CX(2, 1);
	sim.RZ(-0.0245436926062, 1);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.0490873852123, 1);
	sim.CX(1, 0);
	sim.RZ(0.0490873852123, 0);
	sim.CX(1, 0);
	sim.RZ(-0.0490873852123, 0);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.RZ(-0.0981747704247, 4);
	sim.CX(4, 7);
	sim.RZ(0.0981747704247, 7);
	sim.CX(4, 7);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(1, 4);
	sim.RZ(-0.196349540849, 2);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.RZ(-0.0981747704247, 7);
	sim.CX(6, 7);
	sim.CX(7, 6);
	sim.CX(6, 7);
	sim.RZ(-0.00613592315154, 8);
	sim.CX(8, 9);
	sim.CX(9, 8);
	sim.CX(8, 9);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.CX(2, 3);
	sim.RZ(0.196349540849, 3);
	sim.CX(2, 3);
	sim.RZ(-0.392699081699, 2);
	sim.CX(2, 1);
	sim.RZ(0.392699081699, 1);
	sim.CX(2, 1);
	sim.RZ(-0.392699081699, 1);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(1, 4);
	sim.RZ(-0.196349540849, 3);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(0, 1);
	sim.CX(1, 0);
	sim.CX(0, 1);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.785398163397, 4);
	sim.CX(4, 7);
	sim.RZ(0.785398163397, 7);
	sim.CX(4, 7);
	sim.RZ(-0.785398163397, 7);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.RZ(-0.0122718463031, 13);
	sim.CX(6, 7);
	sim.CX(7, 6);
	sim.CX(6, 7);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(8, 9);
	sim.CX(9, 8);
	sim.CX(8, 9);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(13, 14);
	sim.RZ(0.0122718463031, 14);
	sim.CX(13, 14);
	sim.RZ(-0.0122718463031, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.RZ(-0.0245436926062, 14);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(14, 11);
	sim.RZ(0.0245436926062, 11);
	sim.CX(14, 11);
	sim.RZ(-0.0245436926062, 11);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.RZ(-0.0490873852123, 8);
	sim.CX(8, 5);
	sim.RZ(0.0490873852123, 5);
	sim.CX(8, 5);
	sim.RZ(-0.0490873852123, 5);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.RZ(-0.0981747704247, 3);
	sim.CX(3, 2);
	sim.RZ(0.0981747704247, 2);
	sim.CX(3, 2);
	sim.RZ(-0.0981747704247, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.196349540849, 1);
	sim.CX(1, 4);
	sim.CX(3, 5);
	sim.RZ(0.196349540849, 4);
	sim.CX(1, 4);
	sim.RZ(-0.392699081699, 1);
	sim.CX(1, 2);
	sim.RZ(0.392699081699, 2);
	sim.CX(1, 2);
	sim.RZ(-0.785398163397, 1);
	sim.CX(1, 0);
	sim.RZ(0.785398163397, 0);
	sim.CX(1, 0);
	sim.RZ(-0.785398163397, 0);
	sim.CX(0, 1);
	sim.CX(1, 0);
	sim.CX(0, 1);
	sim.RZ(-0.392699081699, 2);
	sim.RZ(-0.196349540849, 4);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(4, 7);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.RZ(-0.0245436926062, 7);
	sim.CX(7, 10);
	sim.RZ(0.0245436926062, 10);
	sim.CX(7, 10);
	sim.RZ(-0.0245436926062, 10);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.RZ(-0.0490873852123, 12);
	sim.CX(12, 13);
	sim.RZ(0.0490873852123, 13);
	sim.CX(12, 13);
	sim.RZ(-0.0490873852123, 13);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.RZ(-0.0981747704247, 14);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(14, 11);
	sim.RZ(0.0981747704247, 11);
	sim.CX(14, 11);
	sim.RZ(-0.0981747704247, 11);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.RZ(-0.196349540849, 8);
	sim.CX(8, 5);
	sim.RZ(0.196349540849, 5);
	sim.CX(8, 5);
	sim.RZ(-0.196349540849, 5);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.RZ(-0.392699081699, 5);
	sim.CX(5, 3);
	sim.RZ(0.392699081699, 3);
	sim.CX(5, 3);
	sim.RZ(-0.392699081699, 3);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.RZ(-0.785398163397, 3);
	sim.CX(3, 2);
	sim.RZ(0.785398163397, 2);
	sim.CX(3, 2);
	sim.RZ(-0.785398163397, 2);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.0490873852123, 1);
	sim.CX(1, 4);
	sim.RZ(0.0490873852123, 4);
	sim.CX(1, 4);
	sim.RZ(-0.0490873852123, 4);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.RZ(-0.0981747704247, 7);
	sim.CX(7, 10);
	sim.RZ(0.0981747704247, 10);
	sim.CX(7, 10);
	sim.RZ(-0.0981747704247, 10);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.RZ(-0.196349540849, 12);
	sim.CX(12, 13);
	sim.RZ(0.196349540849, 13);
	sim.CX(12, 13);
	sim.RZ(-0.196349540849, 13);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.RZ(-0.392699081699, 14);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.CX(8, 11);
	sim.CX(11, 8);
	sim.CX(8, 11);
	sim.CX(14, 11);
	sim.RZ(0.392699081699, 11);
	sim.CX(14, 11);
	sim.RZ(-0.392699081699, 11);
	sim.CX(11, 14);
	sim.CX(14, 11);
	sim.CX(11, 14);
	sim.RZ(-0.785398163397, 11);
	sim.CX(13, 14);
	sim.CX(14, 13);
	sim.CX(13, 14);
	sim.CX(12, 13);
	sim.CX(13, 12);
	sim.CX(12, 13);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(11, 8);
	sim.RZ(0.785398163397, 8);
	sim.CX(11, 8);
	sim.RZ(-0.785398163397, 8);
	sim.CX(5, 8);
	sim.CX(8, 5);
	sim.CX(5, 8);
	sim.CX(3, 5);
	sim.CX(5, 3);
	sim.CX(3, 5);
	sim.RZ(-0.0981747704247, 3);
	sim.CX(3, 2);
	sim.RZ(0.0981747704247, 2);
	sim.CX(3, 2);
	sim.RZ(-0.0981747704247, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.RZ(-0.196349540849, 2);
	sim.CX(2, 1);
	sim.RZ(0.196349540849, 1);
	sim.CX(2, 1);
	sim.RZ(-0.196349540849, 1);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(1, 4);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.CX(2, 3);
	sim.CX(3, 2);
	sim.CX(2, 3);
	sim.RZ(-0.392699081699, 4);
	sim.CX(4, 7);
	sim.RZ(0.392699081699, 7);
	sim.CX(4, 7);
	sim.RZ(-0.392699081699, 7);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.RZ(-0.785398163397, 10);
	sim.CX(10, 12);
	sim.RZ(0.785398163397, 12);
	sim.CX(10, 12);
	sim.RZ(-0.785398163397, 12);
	sim.CX(10, 12);
	sim.CX(12, 10);
	sim.CX(10, 12);
	sim.CX(7, 10);
	sim.CX(10, 7);
	sim.CX(7, 10);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.RZ(-0.196349540849, 4);
	sim.CX(4, 1);
	sim.RZ(0.196349540849, 1);
	sim.CX(4, 1);
	sim.RZ(-0.196349540849, 1);
	sim.CX(1, 4);
	sim.CX(4, 1);
	sim.CX(1, 4);
	sim.RZ(-0.392699081699, 1);
	sim.CX(1, 2);
	sim.RZ(0.392699081699, 2);
	sim.CX(1, 2);
	sim.RZ(-0.785398163397, 1);
	sim.RZ(-0.392699081699, 2);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(1, 4);
	sim.RZ(0.785398163397, 4);
	sim.CX(1, 4);
	sim.CX(1, 2);
	sim.CX(2, 1);
	sim.CX(1, 2);
	sim.RZ(-0.785398163397, 4);
	sim.RZ(-0.392699081699, 4);
	sim.CX(4, 7);
	sim.RZ(0.392699081699, 7);
	sim.CX(4, 7);
	sim.RZ(-0.785398163397, 4);
	sim.CX(4, 1);
	sim.RZ(0.785398163397, 1);
	sim.CX(4, 1);
	sim.RZ(-0.785398163397, 1);
	sim.RZ(-0.785398163397, 1);
	sim.RZ(-0.392699081699, 7);
	sim.CX(4, 7);
	sim.CX(7, 4);
	sim.CX(4, 7);
	sim.CX(1, 4);
	sim.RZ(0.785398163397, 4);
	sim.CX(1, 4);
	sim.RZ(-0.785398163397, 4);
	sim.RZ(1.57079632679, 0);
	sim.SX(0);
	sim.RZ(1.57079632679, 0);
	sim.RZ(1.57079632679, 1);
	sim.SX(1);
	sim.RZ(1.57079632679, 1);
	sim.RZ(1.57079632679, 10);
	sim.SX(10);
	sim.RZ(1.57079632679, 10);
	sim.RZ(1.57079632679, 11);
	sim.SX(11);
	sim.RZ(1.57079632679, 11);
	sim.RZ(1.57079632679, 12);
	sim.SX(12);
	sim.RZ(1.57079632679, 12);
	sim.RZ(1.57079632679, 2);
	sim.SX(2);
	sim.RZ(1.57079632679, 2);
	sim.RZ(1.57079632679, 4);
	sim.SX(4);
	sim.RZ(1.57079632679, 4);
	sim.RZ(1.57079632679, 5);
	sim.SX(5);
	sim.RZ(1.57079632679, 5);
	sim.RZ(1.57079632679, 7);
	sim.SX(7);
	sim.RZ(1.57079632679, 7);
	sim.RZ(1.57079632679, 9);
	sim.SX(9);
	sim.RZ(1.57079632679, 9);
}

int main(int argc, char *argv[])
{
	MPI_Init(&argc, &argv);
	Simulation sim;
	prepare_circuit(sim);
	sim.sim();
	MPI_Finalize();
	return 0;
}
