...
public:
    ...
    void append(Gate& g)
    {
        ...
        printf("%s(theta:%lf,q:%llu,pair_q:%llu, mask:%llu)\n",OP_NAMES[g.op_name], g.theta, g.qubit, g.qubit+n_qubits, g.mask);
#endif
        if (g.qubit >= n_qubits) 
        {
    ...
    IdxType mask_remap(IdxType mask)
    {
        IdxType remapped_mask = 0;
        for (IdxType i=0; i<sizeof(IdxType); i++)
            remapped_mask = remapped_mask | (((mask>>i)&0x1)<<(qmap[i]));
    ...
    Gate* upload()
    {
        /*
        printf("\n=======stat========\n");
        for (int i=0; i<n_qubits; i++) printf("%llu ",stat[i]);
        printf("\n==================\n");
        if (is_reset)
        {
            ...
            printf("\n=======qmap========\n");
            for (int i=0; i<n_qubits; i++) printf("%llu=>%llu ",i,qmap[i]);
            printf("\n==================\n");
        }

        printf("\n=======gate map========\n");
        for (IdxType i=0; i<n_gates; i++)
            printf("%s(%llu)=>%s(%llu), mask-%llu=>mask-%llu\n",OP_NAMES[circuit[i].op_name], circuit[i].qubit, OP_NAMES[circuit[i].op_name], qmap[circuit[i].qubit], circuit[i].mask, mask_remap(circuit[i].mask));
        printf("\n==================\n");
        for (IdxType i=0; i<n_gates; i++)
        {
            circuit[i].qubit = qmap[circuit[i].qubit];
            if (circuit[i].mask != 0 && circuit[i].op_name != Measure) 
                circuit[i].mask = mask_remap(circuit[i].mask);
        }
        */
...
public:
    ...
    ValType sim()
    {
        ...
        unsigned smem_size = THREADS_PER_BLOCK/32*8*16*2*sizeof(ValType);
        int numBlocksPerSm;
        cudaSafeCall(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, 
                    simulation_kernel, THREADS_PER_BLOCK, smem_size));
        ...
        NVSHMEM_CHECK(nvshmemx_collective_launch((const void*)simulation_kernel,gridDim,
                    THREADS_PER_BLOCK,args,smem_size,0));
        cudaSafeCall(cudaDeviceSynchronize());
        ...
        if (i_gpu ==0)
        {
            ...
            printf("\n============== SV-Sim ===============\n");
            printf("nqubits:%llu, ngates:%llu, ngpus:%llu, comp:%.3lf ms, comm:%.3lf ms, sim:%.3lf ms, mem:%.3lf MB, mem_per_gpu:%.3lf MB\n",
...
__device__ void Gate::exe_op(Simulation* sim, ValType* sv_real, ValType* sv_imag)
{
    ...
    if( threadIdx.x==0 && blockIdx.x==0 && (sim->n_qubits-qubit)<=(sim->gpu_scale) ) nvshmem_barrier_all(); 

    (*(this->op))(this, sim, sv_real, sv_imag);
    /* We only need synchronization when the operation is non-local,
       which means 2*n_qubits-qubit<=log2(n_gpus) for DM,
       which means lg2_m_gpu<=qubit of the gate. */

    //if(threadIdx.x==0 && blockIdx.x==0) printf("n_qubit:%llu, qubit:%llu, gpu_scale:%llu\n", sim->n_qubits, qubit, sim->gpu_scale);
    if ((sim->n_qubits-qubit)<=(sim->gpu_scale))
    {
...
__global__ void simulation_kernel(Simulation* sim)
{
    for (IdxType t=0; t<(sim->n_gates); t++)
    {
        ...
        if (sim->i_gpu == 0 && blockIdx.x == 0 && threadIdx.x == 0)
        {
            ...
            printf("%s(%llu) ticks: %llu\n",OP_NAMES_D[sim->circuit_handle_gpu[t].op_name],sim->circuit_handle_gpu[t].qubit, time);
        }
         */
...
#define OP_HEAD grid_group grid = this_grid(); \
    ...
    for (IdxType i=(sim->i_gpu)*per_pe_work+tid; i<(sim->i_gpu+1)*per_pe_work;\
        ...
        //if (qubit >= sim->lg2_m_gpu && sim->i_gpu != pos0_gid && sim->i_gpu != pos1_gid)\
           if (tid==0) printf("============ i_gpu:%llu, q:%llu, pos0_gid:%llu, pos1_gid:%llu\n", sim->i_gpu, qubit, pos0_gid, pos1_gid);
        //if (pos0_gid != sim->i_gpu && pos1_gid != sim->i_gpu) printf("Violation!\n");
...
__device__ __inline__ void X_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, const IdxType qubit)
{
    ...
    if (qubit >= sim->lg2_m_gpu) 
    {
        ...
        //printf("qubit:%llu, per_pe_work:%llu, dim:%llu, i_gpu:%llu, pair_gpu:%llu, flag:%llu \n", qubit, per_pe_work, sim->dim, sim->i_gpu, pair_gpu, flag);

        for (IdxType i=tid; i<per_pe_work; i+=blockDim.x*gridDim.x)
        {
            const ValType el0_real = sv_real_pos0[i];
            const ValType el0_imag = sv_imag_pos0[i];
            const ValType el1_real = sv_real_pos1[i];
            const ValType el1_imag = sv_imag_pos1[i];

            ValType real_pos0 = el1_real;
            ValType imag_pos0 = el1_imag;
            ValType real_pos1 = el0_real;
            ValType imag_pos1 = el0_imag;

            sv_real_pos0[i] = real_pos0;
            sv_imag_pos0[i] = imag_pos0;
            sv_real_pos1[i] = real_pos1;
            sv_imag_pos1[i] = imag_pos1;
        }
        grid.sync();
    ...
    else
    {
        ...
        { 
            ...
            sv_real[pos0] = real_pos0;
            sv_imag[pos0] = imag_pos0;
            sv_real[pos1] = real_pos1;
            sv_imag[pos1] = imag_pos1;
        } 
    }
    grid.sync(); 
}
...
__device__ __inline__ void H_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, const IdxType qubit)
{
    ...
    if (qubit >= sim->lg2_m_gpu) 
    {
        ...
        for (IdxType i=tid; i<per_pe_work; i+=blockDim.x*gridDim.x)
        {
            ...
            ValType real_pos0 =  S2I*(el0_real + el1_real); 
            ValType imag_pos0 =  S2I*(el0_imag + el1_imag);
            ValType real_pos1 =  S2I*(el0_real - el1_real);
            ValType imag_pos1 =  S2I*(el0_imag - el1_imag);
...
//#define OP_HEAD_MASK grid_group grid = this_grid(); \
    ...
    //for (IdxType i=(sim->i_gpu)*per_pe_work+tid; i<(sim->i_gpu+1)*per_pe_work;\
        ...
        //IdxType outer = (idx >> qubit); 
        //IdxType inner =  (idx & (((IdxType)1<<qubit)-1)); 
        //IdxType offset = (outer << (qubit+1)); 
        //IdxType pos0_src = offset + inner; 
        //if (((~(pos0_src&mask))&mask) != 0) continue; 
        ...
        for (IdxType i=tid; i<per_pe_work; i+=blockDim.x*gridDim.x)
        {
            IdxType idx = (sim->i_gpu<< (sim->lg2_m_gpu)) + flag*per_pe_work + i; 
            if (((~(idx&mask))&mask) != 0) continue; 
        ...
        const IdxType qubit2, const IdxType qubit3)
{
            ...
            i+=blockDim.x*gridDim.x) 
    {
        ...
        //const ValType el_real[16] = {0};
        //const ValType el_imag[16] = {0};
        ...
        const IdxType qubit2, const IdxType qubit3)
{
    ...
    //printf("GPU-%llu: dim:%llu, per_pe_work:%llu, nwarps:%d, start:%llu, end:%llu, step:%llu\n",sim->i_gpu, sim->dim,per_pe_work, nwarps,
        ...
        const IdxType qubit2, const IdxType qubit3)
{
    ...
    {
        C4TCV2_GATE(sim, sv_real, sv_imag, CX_real, CX_imag, qubit0, qubit1, qubit2, qubit3); 
        //if (sim->i_gpu == 0 && blockIdx.x == 0 && threadIdx.x == 0)
        //printf("haha!");
    }
    else //s qubit is non-local
    {
        ...
        for (IdxType i=(sim->i_gpu)*per_pe_work+(warpid<<3); i<(sim->i_gpu+1)*per_pe_work;
                i+=(nwarps<<3)) 
        {
            for (int j=0; j<8; j++)
            {
                ...
                if (laneid < 16)
                {
                    ...
                    {
                        if ((j&1)!=0) el_real_s[j*16+laneid] = sv_real_remote[addr];
                        else el_real_s[j*16+laneid] = sv_real[addr];
                    }
                    else //qubit2 is remote (not possible qubit0 or 1 is remote)
                    {
                        if ((j&2)!=0) el_real_s[j*16+laneid] = sv_real_remote[addr];
                        else el_real_s[j*16+laneid] = sv_real[addr];
                    }
                }
                else 
                {
                    ...
                    if (qubit3 == s) //qubit3 is remote qubit
                    {
                        if ((j&1)!=0) el_imag_s[j*16+laneid] = sv_imag_remote[addr];
                        else el_imag_s[j*16+laneid] = sv_imag[addr];
                    }
                    else //qubit2 is remote (not possible qubit0 or 1 is remote)
                    {
                        if ((j&2)!=0) el_imag_s[j*16+laneid] = sv_imag_remote[addr];
                        else el_imag_s[j*16+laneid] = sv_imag[addr];
                    }
                }
            }
...
//============== Unified 4-qubit Gate with TC ================
//This is with tensorcore and nvshmem optimization, and non-blocking comm
__device__ __inline__ void C4TCV4_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, 
        const ValType* gm_real, const ValType* gm_imag, const IdxType qubit0, const IdxType qubit1,
        const IdxType qubit2, const IdxType qubit3)
{
    grid_group grid = this_grid(); 
    ...
    else //s qubit is non-local
    {
        ...
        nvshmemx_double_get_nbi_block(sv_real_remote, sv_real, per_pe_num, pair_gpu );
        nvshmemx_double_get_nbi_block(sv_imag_remote, sv_imag, per_pe_num, pair_gpu );
        ...
        grid.sync();
        nvshmemx_double_put_nbi_block(sv_real, sv_real_remote, per_pe_num, pair_gpu);
        nvshmemx_double_put_nbi_block(sv_imag, sv_imag_remote, per_pe_num, pair_gpu);
...
__device__ void ControlledX_OP(const Gate* g, const Simulation* sim, ValType* sv_real, ValType* sv_imag)
{
    ...
    if (__popcll(g->mask) == 1)
    {
        ...
        //C4TCV2_GATE(sim, sv_real, sv_imag, CX_real, CX_imag, control, g->qubit, control+(sim->n_qubits), (g->qubit)+(sim->n_qubits));


        IdxType t0;
        if (sim->i_gpu == 0 && blockIdx.x == 0 && threadIdx.x == 0) t0 = clock64();

        C4TCV3_GATE(sim, sv_real, sv_imag, CX_real, CX_imag, control, g->qubit, control+(sim->n_qubits), (g->qubit)+(sim->n_qubits));
        ...
        {
            IdxType time = clock64() - t0;
            printf("C4TC(%llu,%llu,%llu,%llu), remote(%llu-%llu) ticks: %llu\n",control, g->qubit, control+(sim->n_qubits), (g->qubit)+(sim->n_qubits), (2*sim->n_qubits)-(sim->gpu_scale), 2*sim->n_qubits-1, time);
        }
        //*/
